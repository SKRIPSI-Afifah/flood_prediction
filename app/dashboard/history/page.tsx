...
type StatusLevel = "error" | "secondary" | "tertiary" | "primary"
...
const statusClasses: Record<StatusLevel, { dot: string; text: string }> = {
  error: {
      ...
      <main className="flex-1 p-8 lg:p-10 space-y-12 max-w-[1600px] mx-auto w-full">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
          {stats.map((stat) => (
            <div
              key={stat.label}
              className="bg-surface-container-lowest p-8 rounded-[40px] shadow-sm border border-surface-container/30 flex flex-col justify-between hover:bg-surface-container transition-colors duration-500 cursor-pointer group"
            >
              ...
              <div className="flex items-baseline gap-3 mt-6">
                <span
                  className={`text-4xl font-black ${stat.color} tracking-tighter group-hover:scale-110 transition-transform duration-500`}
                >
                  {stat.value}
                </span>
                ...
                <div
                  className={`flex items-center gap-1 text-[10px] font-black uppercase tracking-widest ${
                    stat.trendUp ? "text-secondary" : "text-on-surface-variant/60"
                  }`}
                >
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-surface-container-high/30">
                  <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant/40">
                    Waktu
                  </th>
                  <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant/40">
                    Parameter
                  ...
                  <th className="px-10 py-6 text-[10px] font-black uppercase tracking-[0.2em] text-on-surface-variant/40 text-center">
                    Aksi
                  </th>
                </tr>
              </thead>
              ...
              <tbody className="divide-y divide-surface-container">
                {historyRows.map((row) => {
                  const levelClass = statusClasses[row.statusLevel]

                  return (
                    <tr key={`${row.date}-${row.time}`} className="hover:bg-surface-container-low transition-colors group">
                      ...
                      <td className="px-10 py-8">
                        <div className="flex flex-wrap gap-2">
                          {row.params.map((param) => (
                            <span
                              key={param}
                              className="px-3 py-1.5 bg-primary/5 rounded-full text-[9px] font-black text-primary uppercase tracking-widest"
                            >
                              {param}
                            </span>
                          ))}
                      ...
                      <td className="px-10 py-8">
                        <div className="flex items-center gap-3">
                          <div className={`size-3 rounded-full shadow-lg ${levelClass.dot}`} />
                          <span className={`text-[10px] font-black uppercase tracking-widest ${levelClass.text}`}>
                            {row.status}
                          </span>
                        </div>
                      </td>
                      ...
                      <td className="px-10 py-8">
                        <div className="flex items-center justify-center gap-2">
                          <Button
                            variant="ghost"
                            size="icon"
                            className="size-10 rounded-xl hover:bg-white text-primary hover:scale-110 transition-all"
                          >
            ...
            <div className="flex gap-3">
              ...
              {[1, 2, 3].map((page) => (
                <Button
                  ...
                  className={`size-10 rounded-xl font-black text-xs ${
                    page === 1 ? "bg-primary text-white shadow-lg shadow-primary/20" : "hover:bg-white text-primary"
                  }`}
                >
                  {page}
                </Button>
        ...
        <footer className="pt-12 text-center opacity-20 hover:opacity-100 transition-opacity duration-1000">
          <p className="text-[10px] font-black uppercase tracking-[0.4em] text-primary">
            FloodGuard Aceh • Intelligence Division • © 2023
          </p>
        </footer>
      </main>
    </>
